...
> 每个包的地址
> AXI 数据位宽：16 位（代码里），2 Byte，文档中 32 位
> 每个队列独立选择 mmp 或 st，能，好处：可以分别处理实时数据信号或传统 DMA
> 优先级支持：支持加权轮询（Weighted Round Robin）
> 
...
> 颜色位位数：一位
> - 初始化 head 与 tail 为 0，入队 tail++，出队 head++
> - 走完一圈后颜色位反转（硬件上可通过多设置一位用溢出、掩码来实现）
> - 颜色位相同：队头队尾相遇
- CMPT 队列 host 内存中的环
- 消费者是软件，生产者是 CMPT 引擎
